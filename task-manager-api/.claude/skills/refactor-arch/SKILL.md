...
6. Salve o relatório em `reports/audit-project-N.md` na raiz do repositório (pergunte ao usuário qual N usar se não estiver óbvio pelo contexto, ou infira pela ordem: code-smells-project = 1, ecommerce-api-legacy = 2, task-manager-api = 3) e também exiba o relatório completo no terminal.
...
4. Extraia toda configuração sensível (chaves, senhas, connection strings) para variáveis de ambiente carregadas via um módulo de config dedicado, nunca hardcoded no código.
5. Centralize tratamento de erros (middleware/error handler único).
6. Garanta um entry point único e claro (composition root) que monta a aplicação a partir das peças (config, models, controllers, rotas).
7. Depois de mover os arquivos, valide o resultado:
...
9. Nunca finalize a fase 3 alegando sucesso sem ter efetivamente rodado a aplicação e testado os endpoints. Se algo falhar, corrija antes de reportar conclusão.
